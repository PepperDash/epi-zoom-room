...
| Language | C# |
| Declaration | `class ZoomRoomCamera` with declared base/contract list `CameraBase, IHasCameraPtzControl, IHasCameraControls, IBridgeAdvanced` |
| Accessibility | `public` |
| Namespace/module | `PepperDash.Essentials.Plugins` |

## What

`ZoomRoomCamera` is a source-level implementation type with responsibilities defined by its members and inheritance. This description is grounded in its source declaration and declared inheritance rather than inferred product behavior.
...
Preserve the declared inheritance/contract relationship: `CameraBase, IHasCameraPtzControl, IHasCameraControls, IBridgeAdvanced`. Public methods declared in this source file include: `PositionHome`, `PanLeft`, `PanRight`, `PanStop`, `TiltDown`, `TiltUp`, `TiltStop`, `ZoomIn`, `ZoomOut`, `ZoomStop`, `LinkToApi`. Use repository search to identify callers, implementers, serializers, tests, and configuration references before changing a public name or shape.
...
Before proposing a change, read this declaration, its full source file, all repository references to `ZoomRoomCamera`, and its test coverage. Do not invent configuration keys, payload fields, interface members, or lifecycle ordering. Report the affected source files, tests, and consumer boundaries with any proposed change.
