...
| Source file | [`src/ZoomRoomInfo.cs`](../../../src/ZoomRoomInfo.cs) |
| Language | C# |
| Declaration | `class ZoomRoomInfo` with declared base/contract list `VideoCodecInfo` |
| Accessibility | `public` |
| Namespace/module | `PepperDash.Essentials.Plugins` |

## What

`ZoomRoomInfo` is a structured representation of data exchanged between components. This description is grounded in its source declaration and declared inheritance rather than inferred product behavior.
...
Preserve the declared inheritance/contract relationship: `VideoCodecInfo`. Use repository search to identify callers, implementers, serializers, tests, and configuration references before changing a public name or shape.
...
Before proposing a change, read this declaration, its full source file, all repository references to `ZoomRoomInfo`, and its test coverage. Do not invent configuration keys, payload fields, interface members, or lifecycle ordering. Report the affected source files, tests, and consumer boundaries with any proposed change.
