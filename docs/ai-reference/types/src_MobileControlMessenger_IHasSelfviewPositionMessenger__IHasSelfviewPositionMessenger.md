...
| Source file | [`src/MobileControlMessenger/IHasSelfviewPositionMessenger.cs`](../../../src/MobileControlMessenger/IHasSelfviewPositionMessenger.cs) |
| Language | C# |
| Declaration | `class IHasSelfviewPositionMessenger` with declared base/contract list `SelfviewOptionMessengerBase` |
| Accessibility | `public` |
| Namespace/module | `PepperDash.Essentials.AppServer.Messengers` |

## What

`IHasSelfviewPositionMessenger` is a message-handling boundary that translates between transport payloads and application behavior. This description is grounded in its source declaration and declared inheritance rather than inferred product behavior.
...
Preserve the declared inheritance/contract relationship: `SelfviewOptionMessengerBase`. Use repository search to identify callers, implementers, serializers, tests, and configuration references before changing a public name or shape.
