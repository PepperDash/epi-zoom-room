...
| Source file | [`src/MobileControlMessenger/SelfviewOption.cs`](../../../src/MobileControlMessenger/SelfviewOption.cs) |
| Language | C# |
| Declaration | `class SelfviewOption` |
| Accessibility | `public` |
| Namespace/module | `PepperDash.Essentials.AppServer.Messengers` |

## What

`SelfviewOption` is a source-level implementation type with responsibilities defined by its members and inheritance. This description is grounded in its source declaration and declared inheritance rather than inferred product behavior.
...
Before proposing a change, read this declaration, its full source file, all repository references to `SelfviewOption`, and its test coverage. Do not invent configuration keys, payload fields, interface members, or lifecycle ordering. Report the affected source files, tests, and consumer boundaries with any proposed change.
