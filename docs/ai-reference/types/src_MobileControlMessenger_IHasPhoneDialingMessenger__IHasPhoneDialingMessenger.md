...
| Source file | [`src/MobileControlMessenger/IHasPhoneDialingMessenger.cs`](../../../src/MobileControlMessenger/IHasPhoneDialingMessenger.cs) |
| Language | C# |
| Declaration | `class IHasPhoneDialingMessenger` with declared base/contract list `MessengerBase` |
| Accessibility | `public` |
| Namespace/module | `PepperDash.Essentials.AppServer.Messengers` |

## What

`IHasPhoneDialingMessenger` is a message-handling boundary that translates between transport payloads and application behavior. This description is grounded in its source declaration and declared inheritance rather than inferred product behavior.
...
Before proposing a change, read this declaration, its full source file, all repository references to `IHasPhoneDialingMessenger`, and its test coverage. Do not invent configuration keys, payload fields, interface members, or lifecycle ordering. Report the affected source files, tests, and consumer boundaries with any proposed change.
