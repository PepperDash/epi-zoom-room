...
| Source file | [`src/MobileControlMessenger/IHasSelfviewSizeMessenger.cs`](../../../src/MobileControlMessenger/IHasSelfviewSizeMessenger.cs) |
| Language | C# |
| Declaration | `class IHasSelfviewSizeMessenger` with declared base/contract list `SelfviewOptionMessengerBase` |
| Accessibility | `public` |
| Namespace/module | `PepperDash.Essentials.AppServer.Messengers` |

## What

`IHasSelfviewSizeMessenger` is a message-handling boundary that translates between transport payloads and application behavior. This description is grounded in its source declaration and declared inheritance rather than inferred product behavior.
