# IHasMeetingRecordingWithPromptMessenger
...
| Language | C# |
| Declaration | `class IHasMeetingRecordingWithPromptMessenger` with declared base/contract list `MessengerBase` |
| Accessibility | `public` |
| Namespace/module | `PepperDash.Essentials.AppServer.Messengers` |

## What

`IHasMeetingRecordingWithPromptMessenger` is a message-handling boundary that translates between transport payloads and application behavior. This description is grounded in its source declaration and declared inheritance rather than inferred product behavior.
